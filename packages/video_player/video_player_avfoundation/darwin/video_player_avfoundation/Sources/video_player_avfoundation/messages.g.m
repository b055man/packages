...
@interface FVPSetAudioTrackMessage ()
+ (FVPSetAudioTrackMessage *)fromList:(NSArray *)list;
+ (nullable FVPSetAudioTrackMessage *)nullableFromList:(NSArray *)list;
- (NSArray *)toList;
@end
...
@implementation FVPSetAudioTrackMessage
+ (instancetype)makeWithGroupId:(NSInteger )groupId
    trackId:(NSInteger )trackId {
  FVPSetAudioTrackMessage* pigeonResult = [[FVPSetAudioTrackMessage alloc] init];
...
+ (FVPSetAudioTrackMessage *)fromList:(NSArray *)list {
  FVPSetAudioTrackMessage *pigeonResult = [[FVPSetAudioTrackMessage alloc] init];
  pigeonResult.groupId = [GetNullableObjectAtIndex(list, 0) integerValue];
  pigeonResult.trackId = [GetNullableObjectAtIndex(list, 1) integerValue];
  return pigeonResult;
}
+ (nullable FVPSetAudioTrackMessage *)nullableFromList:(NSArray *)list {
  return (list) ? [FVPSetAudioTrackMessage fromList:list] : nil;
}
- (NSArray *)toList {
  return @[
    @(self.groupId),
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 129: 
      return [FVPSetAudioTrackMessage fromList:[self readValue]];
    default:
...
- (void)writeValue:(id)value {
  ...
  } else if ([value isKindOfClass:[FVPSetAudioTrackMessage class]]) {
    [self writeByte:129];
    [self writeValue:[value toList]];
  } else {
...
void SetUpFVPAVFoundationVideoPlayerApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FVPAVFoundationVideoPlayerApi> *api, NSString *messageChannelSuffix) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.video_player_avfoundation.AVFoundationVideoPlayerApi.setAudioTrack", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FVPAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setAudioTrack:forPlayer:error:)], @"FVPAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(setAudioTrack:forPlayer:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FVPSetAudioTrackMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        NSInteger arg_textureId = [GetNullableObjectAtIndex(args, 1) integerValue];
        FlutterError *error;
        [api setAudioTrack:arg_msg forPlayer:arg_textureId error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
